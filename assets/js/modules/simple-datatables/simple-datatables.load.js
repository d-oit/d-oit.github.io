...
document.querySelectorAll('.data-table').forEach(tbl => {
    let modulTableOptions = { };
    let sortable = (tbl.getAttribute('data-table-sortable') === 'true');
    modulTableOptions.sortable = sortable;
    let paging = (tbl.getAttribute('data-table-paging') === 'true');
    modulTableOptions.paging = paging;
    let searchable = (tbl.getAttribute('data-table-searchable') === 'true');
    modulTableOptions.searchable = searchable;
    modulTableOptions.perPage = 20,
    modulTableOptions.perPageSelect = [10, 20, 50, ["{{ T "tablePageSelectAll" }}", -1]],
    modulTableOptions.locale = "{{ site.Language.Lang | default "en" }}",
    modulTableOptions.labels = {
      placeholder: "{{ T "tablePlaceholder" }}",
      searchTitle: "{{ T "tablesSearchTitle" }}",
      perPage: "{{ T "tablesPerPage" }}",
      noRows: "{{ T "tablesNoRows" }}",
    ...
    new window.simpleDatatables.DataTable(tbl, modulTableOptions)
})
