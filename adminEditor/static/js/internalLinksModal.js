...
function insertInternalLink () {
  ...
  if (lang === '-') {
    window.alert('Please select a language and file')
    return
  }
  ...
  let showButton = document.getElementById('flexSwitchUseButton').value
  showButton = showButton === 'on' ? 'true' : 'false'
  const selectButtonColorElement = document.getElementById('buttonColorSelect')
  const selectedButtonColorValue = selectButtonColorElement.options[selectButtonColorElement.selectedIndex].value
