...
(function (global, factory) {
  ...
  const scrollerGap = 50

  // Returned or thrown by various protocols to signal 'I'm not
  // handling this'.
  const Pass = { toString: function () { return 'CodeMirror.Pass' } }

  // Reused option objects for setSelection & friends
  const sel_dontScroll = { scroll: false }; const sel_mouse = { origin: '*mouse' }; const sel_move = { origin: '+move' }
  ...
  function insertSorted (array, value, score) {
    let pos = 0; const priority = score(value)
  ...
  function findFirst (pred, from, to) {
    ...
    for (;;) {
      ...
      const midF = (from + to) / 2; const mid = dir < 0 ? Math.ceil(midF) : Math.floor(midF)
  ...
  const bidiOrdering = (function () {
    ...
    function charType (code) {
      if (code <= 0xf7) { return lowTypes.charAt(code) } else if (code >= 0x590 && code <= 0x5f4) { return 'R' } else if (code >= 0x600 && code <= 0x6f9) { return arabicTypes.charAt(code - 0x600) } else if (code >= 0x6ee && code <= 0x8ac) { return 'r' } else if (code >= 0x2000 && code <= 0x200b) { return 'w' } else if (code == 0x200c) { return 'b' } else { return 'L' }
    }

    const bidiRE = /[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/
    const isNeutral = /[stwN]/; const isStrong = /[LRr]/; const countsAsLeft = /[Lb1n]/; const countsAsNum = /[1n]/
    ...
    return function (str, direction) {
      ...
      const len = str.length; const types = []
      ...
      for (let i$7 = 0; i$7 < len;) {
        if (countsAsLeft.test(types[i$7])) {
          ...
          let pos = i$7; let at = order.length; const isRTL = direction == 'rtl' ? 1 : 0
  ...
  function off (emitter, type, f) {
    if (emitter.removeEventListener) {
      ...
      const map = emitter._handlers; const arr = map && map[type]
      if (arr) {
  ...
  const splitLinesAuto = '\n\nb'.split(/\n/).length != 3
    ? function (string) {
      let pos = 0; const result = []; const l = string.length
      while (pos <= l) {
  ...
  function getLine (doc, n) {
    ...
    while (!chunk.lines) {
      for (let i = 0; ; ++i) {
        const child = chunk.children[i]; const sz = child.chunkSize()
  ...
  function lineNo (line) {
    ...
    let cur = line.parent; let no = indexOf(cur.lines, line)
  ...
  function lineAtHeight (chunk, h) {
    ...
    outer: do {
      for (let i$1 = 0; i$1 < chunk.children.length; ++i$1) {
        const child = chunk.children[i$1]; const ch = child.height
    ...
    for (; i < chunk.lines.length; ++i) {
      const line = chunk.lines[i]; const lh = line.height
      if (h < lh) { break }
      h -= lh
    }
    return n + i
  ...
  function highlightLine (cm, line, context, forceToEnd) {
    ...
    const st = [cm.state.modeGen]; const lineClasses = {}
    ...
    const loop = function (o) {
      ...
      const overlay = cm.state.overlays[o]; let i = 1; let at = 0
      context.state = true
  ...
  function getContextBefore (cm, n, precise) {
    const doc = cm.doc; const display = cm.display
  ...
  function takeToken (cm, pos, precise, asArray) {
    const doc = cm.doc; const mode = doc.mode; let style
    pos = clipPos(doc, pos)
    const line = getLine(doc, pos.line); const context = getContextBefore(cm, pos.line, precise)
    const stream = new StringStream(line.text, cm.options.tabSize, context); let tokens
    if (asArray) { tokens = [] }
  ...
  function runMode (cm, text, mode, context, f, lineClasses, forceToEnd) {
    ...
    let curStart = 0; let curStyle = null
    const stream = new StringStream(text, cm.options.tabSize, context); let style
  ...
  function findStartLine (cm, n, precise) {
    let minindent; let minline; const doc = cm.doc
    const lim = precise ? -1 : n - (cm.doc.mode.innerMode ? 1000 : 100)
    for (let search = n; search > lim; --search) {
      if (search <= doc.first) { return doc.first }
      const line = getLine(doc, search - 1); const after = line.stateAfter
  ...
  let sawReadOnlySpans = false; let sawCollapsedSpans = false
  ...
  function addMarkedSpan (line, span, op) {
    const inThisOp = op && window.WeakSet && (op.markedSpans || (op.markedSpans = new WeakSet()))
  ...
  function markedSpansBefore (old, startCh, isInsert) {
    ...
    if (old) {
      for (let i = 0; i < old.length; ++i) {
        const span = old[i]; const marker = span.marker
  ...
  function stretchSpansOverChange (doc, change) {
    ...
    const startCh = change.from.ch; const endCh = change.to.ch; const isInsert = cmp(change.from, change.to) == 0
    // Get the spans that 'stick out' on both sides
    let first = markedSpansBefore(oldFirst, startCh, isInsert)
    let last = markedSpansAfter(oldLast, endCh, isInsert)

    // Next, merge those two ends
    const sameLine = change.text.length == 1; const offset = lst(change.text).length + (sameLine ? startCh : 0)
    if (first) {
    ...
    if (!sameLine) {
      ...
      const gap = change.text.length - 2; let gapMarkers
      if (gap > 0 && first) {
  ...
  function removeReadOnlyRanges (doc, from, to) {
    ...
    for (let i = 0; i < markers.length; ++i) {
      const mk = markers[i]; const m = mk.find(0)
      for (let j = 0; j < parts.length; ++j) {
        const p = parts[j]
        if (cmp(p.to, m.from) < 0 || cmp(p.from, m.to) > 0) { continue }
        const newParts = [j, 1]; const dfrom = cmp(p.from, m.from); const dto = cmp(p.to, m.to)
  ...
  function compareCollapsedMarkers (a, b) {
    ...
    const aPos = a.find(); const bPos = b.find()
  ...
  function collapsedSpanAtSide (line, start) {
    const sps = sawCollapsedSpans && line.markedSpans; let found
    if (sps) {
  ...
  function visualLineNo (doc, lineN) {
    const line = getLine(doc, lineN); const vis = visualLine(line)
  ...
  function visualLineEndNo (doc, lineN) {
    ...
    let line = getLine(doc, lineN); let merged
  ...
  function heightAtLine (lineObj) {
    ...
    let h = 0; let chunk = lineObj.parent
  ...
  function lineLength (line) {
    ...
    let len = line.text.length; let merged; let cur = line
  ...
  function findMaxLine (cm) {
    const d = cm.display; const doc = cm.doc
  ...
  const styleToClassCache = {}; const styleToClassCacheWithMode = {}
  ...
  function buildLineContent (cm, lineView) {
    ...
    for (let i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      const line = i ? lineView.rest[i - 1] : lineView.line; let order = (void 0)
      builder.pos = 0
  ...
  function buildToken (builder, text, style, startStyle, endStyle, css, attributes) {
    ...
    const special = builder.cm.state.specialChars; let mustWrap = false
    let content
    if (!special.test(text)) {
      ...
      while (true) {
        ...
        let txt$1 = (void 0)
        if (m[0] == '\t') {
          const tabSize = builder.cm.options.tabSize; const tabWidth = tabSize - builder.col % tabSize
  ...
  function splitSpaces (text, trailingBefore) {
    ...
    let spaceBefore = trailingBefore; let result = ''
  ...
  function buildTokenBadBidi (inner, order) {
    return function (builder, text, style, startStyle, endStyle, css, attributes) {
      ...
      let start = builder.pos; const end = start + text.length
      for (;;) {
  ...
  function insertLineContent (line, builder, styles) {
    const spans = line.markedSpans; const allText = line.text; let at = 0
    if (!spans) {
    ...
    const len = allText.length; let pos = 0; let i = 1; let text = ''; let style; let css
    let nextChange = 0; let spanStyle; let spanEndStyle; let spanStartStyle; let collapsed; let attributes
    for (;;) {
      if (nextChange == pos) { // Update current marker set
        spanStyle = spanEndStyle = spanStartStyle = css = ''
        attributes = null
        collapsed = null; nextChange = Infinity
        const foundBookmarks = []; let endStyles = (void 0)
        for (let j = 0; j < spans.length; ++j) {
          const sp = spans[j]; const m = sp.marker
  ...
  function fireCallbacksForOps (group) {
    ...
    // new ones appear
    const callbacks = group.delayedCallbacks; let i = 0
    do {
  ...
  function signalLater (emitter, type /*, values... */) {
    ...
    const args = Array.prototype.slice.call(arguments, 2); let list
    if (operationGroup) {
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (markers) {
        for (let k = 0; k < cm.display.gutterSpecs.length; ++k) {
          const id = cm.display.gutterSpecs[k].className; const found = markers.hasOwnProperty(id) && markers[id]
          if (found) {
  ...
  function insertLineWidgetsFor (cm, line, lineView, dims, allowAbove) {
    ...
    for (let i = 0, ws = line.widgets; i < ws.length; ++i) {
      const widget = ws[i]; const node = elt('div', [widget.node], 'CodeMirror-linewidget' + (widget.className ? ' ' + widget.className : ''))
    ...
    if (!lineView.measure.heights || wrapping && lineView.measure.width != curWidth) {
      ...
      if (wrapping) {
        ...
        for (let i = 0; i < rects.length - 1; i++) {
          const cur = rects[i]; const next = rects[i + 1]
  ...
  const nullRect = { left: 0, right: 0, top: 0, bottom: 0 }
  ...
  function measureCharInner (cm, prepared, ch, bias) {
    ...
    const node = place.node; let start = place.start; let end = place.end; let collapse = place.collapse
    ...
    const rtop = rect.top - prepared.rect.top; const rbot = rect.bottom - prepared.rect.top
    const mid = (rtop + rbot) / 2
    const heights = prepared.view.measure.heights
    let i = 0
    for (; i < heights.length - 1; i++) { if (mid < heights[i]) { break } }
    const top = i ? heights[i - 1] : 0; const bot = heights[i]
    const result = {
  ...
  function fromCoordSystem (cm, coords, context) {
    ...
    let left = coords.left; let top = coords.top
  ...
  function cursorCoords (cm, pos, context, lineObj, preparedMeasure, varHeight) {
    ...
    const order = getOrder(lineObj, cm.doc.direction); let ch = pos.ch; let sticky = pos.sticky
    ...
    function getBidi (ch, partPos, invert) {
      const part = order[partPos]; const right = part.level == 1
  ...
  function coordsChar (cm, x, y) {
    ...
    let lineN = lineAtHeight(doc, y); const last = doc.first + doc.size - 1
  ...
  function wrappedLineExtent (cm, lineObj, preparedMeasure, y) {
    ...
    const begin = findFirst(function (ch) { return measureCharPrepared(cm, preparedMeasure, ch - 1).bottom <= y }, end, 0)
  ...
  function coordsCharInner (cm, lineObj, lineNo, x, y) {
    ...
    const widgetHeight = widgetTopHeight(lineObj)
    let begin = 0; let end = lineObj.text.length; let ltr = true

    const order = getOrder(lineObj, cm.doc.direction)
    // If the line isn't plain left-to-right text, first figure out
    // which bidi section the coordinates fall into.
    if (order) {
      const part = (cm.options.lineWrapping ? coordsBidiPartWrapped : coordsBidiPart)
    ...
    let chAround = null; let boxAround = null
    ...
    let baseX; let sticky; let outside = false
    // If a box around the coordinates was found, use that
    if (boxAround) {
      // Distinguish coordinates nearer to the left or right side of the box
      const atLeft = x - boxAround.left < boxAround.right - x; const atStart = atLeft == ltr
  ...
  function coordsBidiPart (cm, lineObj, lineNo, preparedMeasure, order, x, y) {
    ...
    // coordinates.
    const index = findFirst(function (i) {
      const part = order[i]; const ltr = part.level != 1
    ...
    if (index > 0) {
      ...
      const start = cursorCoords(cm, Pos(lineNo, ltr ? part.from : part.to, ltr ? 'after' : 'before'),
  ...
  function coordsBidiPartWrapped (cm, lineObj, _lineNo, preparedMeasure, order, x, y) {
    ...
    const begin = ref.begin
    let end = ref.end
    if (/\s/.test(lineObj.text.charAt(end - 1))) { end-- }
    let part = null; let closestDist = null
    for (let i = 0; i < order.length; i++) {
      const p = order[i]
  ...
  function charWidth (display) {
    ...
    const rect = anchor.getBoundingClientRect(); const width = (rect.right - rect.left) / 10
  ...
  function getDimensions (cm) {
    const d = cm.display; const left = {}; const width = {}
  ...
  function estimateHeight (cm) {
    const th = textHeight(cm.display); const wrapping = cm.options.lineWrapping
  ...
  function estimateLineHeights (cm) {
    const doc = cm.doc; const est = estimateHeight(cm)
    doc.iter(function (line) {
  ...
  function posFromMouse (cm, e, liberal, forRect) {
    ...
    let x; let y; const space = display.lineSpace.getBoundingClientRect()
    // Fails unpredictably on IE[67] when mouse is dragged around quickly.
    try { x = e.clientX - space.left; y = e.clientY - space.top } catch (e$1) { return null }
    let coords = coordsChar(cm, x, y); let line
  ...
  function regLineChange (cm, line, type) {
    cm.curOp.viewChanged = true
    const display = cm.display; const ext = cm.display.externalMeasured
  ...
  function viewCuttingPoint (cm, oldN, newN, dir) {
    let index = findViewIndex(cm, oldN); let diff; const view = cm.display.view
  ...
  function adjustView (cm, from, to) {
    const display = cm.display; const view = display.view
  ...
  function countDirtyView (cm) {
    const view = cm.display.view; let dirty = 0
    for (let i = 0; i < view.length; i++) {
  ...
  function drawSelectionRange (cm, range, output) {
    const display = cm.display; const doc = cm.doc
    const fragment = document.createDocumentFragment()
    const padding = paddingH(cm.display); const leftSide = padding.left
    ...
    function drawForLine (line, fromArg, toArg) {
      ...
      iterateBidiSections(order, fromArg || 0, toArg == null ? lineLen : toArg, function (from, to, dir, i) {
        ...
        const openStart = fromArg == null && from == 0; const openEnd = toArg == null && to == lineLen
        const first = i == 0; const last = !order || i == order.length - 1
    ...
    const sFrom = range.from(); const sTo = range.to()
    if (sFrom.line == sTo.line) {
      drawForLine(sFrom.line, sFrom.ch, sTo.ch)
    } else {
      const fromLine = getLine(doc, sFrom.line); const toLine = getLine(doc, sTo.line)
  ...
  function updateHeightsInViewport (cm) {
    ...
    for (let i = 0; i < display.view.length; i++) {
      const cur = display.view[i]; const wrapping = cm.options.lineWrapping
      let height = (void 0); let width = 0
  ...
  function updateWidgetHeight (line) {
    if (line.widgets) {
      for (let i = 0; i < line.widgets.length; ++i) {
        const w = line.widgets[i]; const parent = w.node.parentNode
  ...
  function visibleLines (display, doc, viewport) {
    ...
    let from = lineAtHeight(doc, top); let to = lineAtHeight(doc, bottom)
    // Ensure is a {from: {line, ch}, to: {line, ch}} object, and
    // forces those lines into the viewport (if possible).
    if (viewport && viewport.ensure) {
      const ensureFrom = viewport.ensure.from.line; const ensureTo = viewport.ensure.to.line
      if (ensureFrom < from) {
        from = ensureFrom
  ...
  function maybeScrollWindow (cm, rect) {
    ...
    const display = cm.display; const box = display.sizer.getBoundingClientRect(); let doScroll = null
  ...
  function scrollPosIntoView (cm, pos, end, margin) {
    ...
    for (let limit = 0; limit < 5; limit++) {
      ...
      const startTop = cm.doc.scrollTop; const startLeft = cm.doc.scrollLeft
  ...
  function calculateScrollPos (cm, rect) {
    const display = cm.display; const snapMargin = textHeight(cm.display)
    if (rect.top < 0) { rect.top = 0 }
    const screentop = cm.curOp && cm.curOp.scrollTop != null ? cm.curOp.scrollTop : display.scroller.scrollTop
    const screen = displayHeight(cm); const result = {}
    if (rect.bottom - rect.top > screen) { rect.bottom = rect.top + screen }
    const docBottom = cm.doc.height + paddingVert(display)
    const atTop = rect.top < snapMargin; const atBottom = rect.bottom > docBottom - snapMargin
    ...
    const gutterSpace = cm.options.fixedGutter ? 0 : display.gutters.offsetWidth
    const screenleft = cm.curOp && cm.curOp.scrollLeft != null ? cm.curOp.scrollLeft : display.scroller.scrollLeft - gutterSpace
    const screenw = displayWidth(cm) - display.gutters.offsetWidth
  ...
  function resolveScrollToPos (cm) {
    ...
    if (range) {
      ...
      const from = estimateCoords(cm, range.from); const to = estimateCoords(cm, range.to)
  ...
  function measureForScrollbars (cm) {
    const d = cm.display; const gutterW = d.gutters.offsetWidth
  ...
  function updateScrollbars (cm, measure) {
    ...
    let startWidth = cm.display.barWidth; let startHeight = cm.display.barHeight
  ...
  function endOperation_R1 (op) {
    const cm = op.cm; const display = cm.display
  ...
  function endOperation_finish (op) {
    const cm = op.cm; const display = cm.display; const doc = cm.doc
    ...
    // undoing
    const hidden = op.maybeHiddenMarkers; const unhidden = op.maybeUnhiddenMarkers
    if (hidden) {
  ...
  function highlightWorker (cm) {
    ...
    doc.iter(context.line, Math.min(doc.first + doc.size, cm.display.viewTo + 500), function (line) {
      if (context.line >= cm.display.viewFrom) { // Visible
        ...
        const oldCls = line.styleClasses; const newCls = highlighted.classes
  ...
  function patchDisplay (cm, updateNumbersFrom, dims) {
    const display = cm.display; const lineNumbers = cm.options.lineNumbers
    const container = display.lineDiv; let cur = container.firstChild
    ...
    const view = display.view; let lineN = display.viewFrom
  ...
  function alignHorizontally (cm) {
    ...
    const gutterW = display.gutters.offsetWidth; const left = comp + 'px'
    for (let i = 0; i < view.length; i++) {
      if (!view[i].hidden) {
  ...
  function maybeUpdateLineNumberWidth (cm) {
    ...
    const doc = cm.doc; const last = lineNumberFor(cm.options, doc.first + doc.size - 1); const display = cm.display
    if (last.length != display.lineNumChars) {
      const test = display.measure.appendChild(elt('div', [elt('div', last)],
        'CodeMirror-linenumber CodeMirror-gutter-elt'))
      const innerW = test.firstChild.offsetWidth; const padding = test.offsetWidth - innerW
  ...
  function getGutters (gutters, lineNumbers) {
    const result = []; let sawLineNumbers = false
    for (let i = 0; i < gutters.length; i++) {
      let name = gutters[i]; let style = null
  ...
  function renderGutters (display) {
    const gutters = display.gutters; const specs = display.gutterSpecs
  ...
  let wheelSamples = 0; let wheelPixelsPerUnit = null
  ...
  function wheelEventDelta (e) {
    let dx = e.wheelDeltaX; let dy = e.wheelDeltaY
  ...
  function onScrollWheel (cm, e) {
    const delta = wheelEventDelta(e); const dx = delta.x; const dy = delta.y

    const display = cm.display; const scroll = display.scroller
    ...
    if (dy && wheelPixelsPerUnit != null) {
      ...
      let top = cm.doc.scrollTop; let bot = top + display.wrapper.clientHeight
  ...
  Selection.prototype.equals = function (other) {
    ...
    for (let i = 0; i < this.ranges.length; i++) {
      const here = this.ranges[i]; const there = other.ranges[i]
  ...
  function normalizeSelection (cm, ranges, primIndex) {
    ...
    for (let i = 1; i < ranges.length; i++) {
      const cur = ranges[i]; const prev = ranges[i - 1]
      const diff = cmp(prev.to(), cur.from())
      if (mayTouch && !cur.empty() ? diff > 0 : diff >= 0) {
        const from = minPos(prev.from(), cur.from()); const to = maxPos(prev.to(), cur.to())
  ...
  function adjustForChange (pos, change) {
    ...
    const line = pos.line + change.text.length - (change.to.line - change.from.line) - 1; let ch = pos.ch
  ...
  function computeReplacedSel (doc, changes, hint) {
    const out = []
    let oldPrev = Pos(doc.first, 0); let newPrev = oldPrev
    for (let i = 0; i < changes.length; i++) {
      ...
      newPrev = to
      if (hint == 'around') {
        const range = doc.sel.ranges[i]; const inv = cmp(range.head, range.anchor) < 0
  ...
  function updateDoc (doc, change, markedSpans, estimateHeight) {
    ...
    const from = change.from; const to = change.to; var text = change.text
    const firstLine = getLine(doc, from.line); const lastLine = getLine(doc, to.line)
    const lastText = lst(text); const lastSpans = spansFor(text.length - 1); const nlines = to.line - from.line
  ...
  function addSelectionToHistory (doc, sel, opId, options) {
    const hist = doc.history; const origin = options && options.origin
  ...
  function attachLocalSpans (doc, change, from, to) {
    let existing = change['spans_' + doc.id]; let n = 0
  ...
  function mergeOldSpans (doc, change) {
    ...
    for (let i = 0; i < old.length; ++i) {
      const oldCur = old[i]; const stretchCur = stretched[i]
  ...
  function copyHistoryArray (events, newGroup, instantiateSel) {
    ...
    for (let i = 0; i < events.length; ++i) {
      ...
      const changes = event.changes; const newChanges = []
      copy.push({ changes: newChanges })
      for (let j = 0; j < changes.length; ++j) {
        const change = changes[j]; let m = (void 0)
  ...
  function setSelectionReplaceHistory (doc, sel, options) {
    const done = doc.history.done; const last = lst(done)
    if (last && last.ranges) {
  ...
  function skipAtomicInner (doc, pos, oldPos, dir, mayClear) {
    ...
    if (line.markedSpans) {
      for (let i = 0; i < line.markedSpans.length; ++i) {
        const sp = line.markedSpans[i]; const m = sp.marker
        ...
        if ((sp.from == null || (preventCursorLeft ? sp.from <= pos.ch : sp.from < pos.ch)) &&
          ...
          if (oldPos) {
            let near = m.find(dir < 0 ? 1 : -1); let diff = (void 0)
  ...
  function makeChange (doc, change, ignoreReadOnly) {
    ...
    if (split) {
      for (let i = split.length - 1; i >= 0; --i) { makeChangeInner(doc, { from: split[i].from, to: split[i].to, text: i ? [''] : change.text, origin: change.origin }) }
    } else {
  ...
  function makeChangeFromHistory (doc, type, allowSelectionOnly) {
    ...
    const hist = doc.history; let event; let selAfter = doc.sel
    const source = type == 'undo' ? hist.done : hist.undone; const dest = type == 'undo' ? hist.undone : hist.done
  ...
  function makeChangeSingleDocInEditor (cm, change, spans) {
    const doc = cm.doc; const display = cm.display; const from = change.from; const to = change.to

    let recomputeMaxLength = false; let checkWidthStart = from.line
    ...
    const changesHandler = hasHandler(cm, 'changes'); const changeHandler = hasHandler(cm, 'change')
  ...
  function rebaseHist (hist, change) {
    const from = change.from.line; const to = change.to.line; const diff = change.text.length - (to - from) - 1
  ...
  BranchChunk.prototype = {
    ...
    removeInner: function (at, n) {
      ...
      for (let i = 0; i < this.children.length; ++i) {
        const child = this.children[i]; const sz = child.chunkSize()
        if (at < sz) {
          const rm = Math.min(n, sz - at); const oldHeight = child.height
  ...
  LineWidget.prototype.clear = function () {
    const cm = this.doc.cm; const ws = this.line.widgets; const line = this.line; const no = lineNo(line)
  ...
  LineWidget.prototype.changed = function () {
    ...
    const oldH = this.height; const cm = this.doc.cm; const line = this.line
    this.height = null
  ...
  TextMarker.prototype.clear = function () {
    ...
    const cm = this.doc.cm; const withOp = cm && !cm.curOp
    ...
    for (let i = 0; i < this.lines.length; ++i) {
      const line = this.lines[i]
      const span = getMarkedSpanFor(line.markedSpans, this)
    ...
    if (cm && this.collapsed && !cm.options.lineWrapping) {
      for (let i$1 = 0; i$1 < this.lines.length; ++i$1) {
        const visual = visualLine(this.lines[i$1]); const len = lineLength(visual)
  ...
  TextMarker.prototype.changed = function () {
    ...
    const pos = this.find(-1, true); const widget = this; const cm = this.doc.cm
    if (!pos || !cm) { return }
    runInOp(cm, function () {
      const line = pos.line; const lineN = lineNo(pos.line)
  ...
  function markText (doc, from, to, options, type) {
    ...
    const marker = new TextMarker(doc, type); const diff = cmp(from, to)
    ...
    let curLine = from.line; const cm = doc.cm; let updateMaxLine
  ...
  function markTextShared (doc, from, to, options, type) {
    ...
    const markers = [markText(doc, from, to, options, type)]; let primary = markers[0]
  ...
  function copySharedMarkers (doc, markers) {
    for (let i = 0; i < markers.length; i++) {
      const marker = markers[i]; const pos = marker.find()
      const mFrom = doc.clipPos(pos.from); const mTo = doc.clipPos(pos.to)
      if (cmp(mFrom, mTo)) {
  ...
  function detachSharedMarkers (markers) {
    const loop = function (i) {
      const marker = markers[i]; const linked = [marker.primary.doc]
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    setValue: docMethodOp(function (code) {
      const top = Pos(this.first, 0); const last = this.first + this.size - 1
      makeChange(this, {
        from: top,
    ...
    getLine: function (line) { const l = this.getLineHandle(line); return l && l.text },
    ...
    getCursor: function (start) {
      const range = this.sel.primary(); let pos
    ...
    getSelection: function (lineSep) {
      const ranges = this.sel.ranges; let lines
      for (let i = 0; i < ranges.length; i++) {
    ...
    getSelections: function (lineSep) {
      const parts = []; const ranges = this.sel.ranges
      for (let i = 0; i < ranges.length; i++) {
    ...
    replaceSelections: docMethodOp(function (code, collapse, origin) {
      const changes = []; const sel = this.sel
      for (let i = 0; i < sel.ranges.length; i++) {
        const range = sel.ranges[i]
        changes[i] = { from: range.from(), to: range.to(), text: this.splitLines(code[i]), origin }
      }
      const newSel = collapse && collapse != 'end' && computeReplacedSel(this, changes, collapse)
      for (let i$1 = changes.length - 1; i$1 >= 0; i$1--) { makeChange(this, changes[i$1]) }
    ...
    historySize: function () {
      const hist = this.history; let done = 0; let undone = 0
    ...
    setHistory: function (histData) {
      const hist = this.history = new History(this.history)
    ...
    findMarksAt: function (pos) {
      pos = clipPos(this, pos)
      const markers = []; const spans = getLine(this, pos.line).markedSpans
      if (spans) {
        for (let i = 0; i < spans.length; ++i) {
          const span = spans[i]
    ...
    findMarks: function (from, to, filter) {
      ...
      const found = []; let lineNo = from.line
    ...
    posFromIndex: function (off) {
      let ch; let lineNo = this.first; const sepSize = this.lineSeparator().length
      this.iter(function (line) {
    ...
    linkedDoc: function (options) {
      if (!options) { options = {} }
      let from = this.first; let to = this.first + this.size
  ...
  function onDrop (e) {
    ...
    let pos = posFromMouse(cm, e, true); const files = e.dataTransfer.files
    if (!pos || cm.isReadOnly()) { return }
    // Might be a file drop, in which case we simply extract the text
    // and insert it.
    if (files && files.length && window.FileReader && window.File) {
      const n = files.length; const text = Array(n); let read = 0
  ...
  function forEachCodeMirror (f) {
    ...
    const byClass = document.getElementsByClassName('CodeMirror'); const editors = []
    ...
    for (const keyname in keymap) {
      if (keymap.hasOwnProperty(keyname)) {
        ...
        for (let i = 0; i < keys.length; i++) {
          let val = (void 0); let name = (void 0)
          if (i == keys.length - 1) {
            name = keys.join(' ')
            val = value
          } else {
  ...
  function deleteNearSelection (cm, compute) {
    const ranges = cm.doc.sel.ranges; const kill = []
  ...
  function moveVisually (cm, line, start, dir) {
    ...
    const partPos = getBidiPartAt(bidi, start.ch, start.sticky); const part = bidi[partPos]
  ...
  const commands = {
    ...
    insertSoftTab: function (cm) {
      const spaces = []; const ranges = cm.listSelections(); const tabSize = cm.options.tabSize
      for (let i = 0; i < ranges.length; i++) {
    ...
    transposeChars: function (cm) {
      return runInOp(cm, function () {
        const ranges = cm.listSelections(); const newSel = []
        for (let i = 0; i < ranges.length; i++) {
          if (!ranges[i].empty()) { continue }
          let cur = ranges[i].head; const line = getLine(cm.doc, cur.line).text
          if (line) {
  ...
  function lineStartSmart (cm, pos) {
    ...
    if (!order || order[0].level == 0) {
      const firstNonWS = Math.max(start.ch, line.text.search(/\S/))
  ...
  function doHandleBinding (cm, bound, dropShift) {
    ...
    const prevShift = cm.display.shift; let done = false
    try {
  ...
  function onKeyPress (e) {
    ...
    const keyCode = e.keyCode; const charCode = e.charCode
  ...
  function onMouseDown (e) {
    const cm = this; const display = cm.display
    ...
    const pos = posFromMouse(cm, e); const button = e_button(e); const repeat = pos ? clickRepeat(pos, button) : 'single'
    window.focus()
  ...
  function leftButtonDown (cm, pos, repeat, event) {
    ...
    const sel = cm.doc.sel; let contained
  ...
  function leftButtonStartDrag (cm, event, pos, behavior) {
    const display = cm.display; let moved = false
    var dragEnd = operation(cm, function (e) {
      ...
      if (!moved) {
        ...
        if ((webkit && !safari) || ie && ie_version == 9) { setTimeout(function () { display.wrapper.ownerDocument.body.focus({ preventScroll: true }); display.input.focus() }, 20) } else { display.input.focus() }
      }
    })
  ...
  function leftButtonSelect (cm, event, start, behavior) {
    ...
    let ourRange; let ourIndex; let startSel = doc.sel; const ranges = startSel.ranges
    ...
    function extendTo (pos) {
      ...
      if (behavior.unit == 'rectangle') {
        const ranges = []; const tabSize = cm.options.tabSize
        ...
        const left = Math.min(startCol, posCol); const right = Math.max(startCol, posCol)
        for (let line = Math.min(start.line, pos.line), end = Math.min(cm.lastLine(), Math.max(start.line, pos.line));
          line <= end; line++) {
          const text = getLine(doc, line).text; const leftPos = findColumn(text, left, tabSize)
        ...
        let anchor = oldRange.anchor; let head
  ...
  function bidiSimplify (cm, range) {
    ...
    const index = getBidiPartAt(order, anchor.ch, anchor.sticky); const part = order[index]
    ...
    const ch = from ? usePart.from : usePart.to; const sticky = from ? 'after' : 'before'
  ...
  function gutterEvent (cm, e, type, prevent) {
    ...
    if (e.touches) {
      ...
      try { mX = e.clientX; mY = e.clientY } catch (e$1) { return false }
    }
  ...
  const Init = { toString: function () { return 'CodeMirror.Init' } }
  ...
  function defineOptions (CodeMirror) {
    ...
    option('lineSeparator', null, function (cm, val) {
      cm.doc.lineSep = val
      if (!val) { return }
      const newBreaks = []; let lineNo = cm.doc.first
      cm.doc.iter(function (line) {
        for (let pos = 0; ;) {
  ...
  function registerEventHandlers (cm) {
    ...
    let touchFinished; let prevTouch = { end: 0 }
    ...
    function farAway (touch, other) {
      ...
      const dx = other.left - touch.left; const dy = other.top - touch.top
    ...
    on(d.scroller, 'touchend', function (e) {
      ...
      if (touch && !eventInWidget(d, e) && touch.left != null &&
        ...
        const pos = cm.coordsChar(d.activeTouch, 'page'); let range
  ...
  function indentLine (cm, n, how, aggressive) {
    ...
    const line = getLine(doc, n); const curSpace = countColumn(line.text, null, tabSize)
    if (line.stateAfter) { line.stateAfter = null }
    const curSpaceString = line.text.match(/^\s*/)[0]; let indentation
  ...
  function applyTextInput (cm, inserted, deleted, sel, origin) {
    ...
    const textLines = splitLinesAuto(inserted); let multiPaste = null
    ...
    const updateInput = cm.curOp.updateInput
  ...
  function addEditorMethods (CodeMirror) {
    ...
    CodeMirror.prototype = {
      ...
      setOption: function (option, value) {
        const options = this.options; const old = options[option]
      ...
      indentSelection: methodOp(function (how) {
        const ranges = this.doc.sel.ranges; let end = -1
        for (let i = 0; i < ranges.length; i++) {
          ...
          if (!range.empty()) {
            ...
            const newRanges = this.doc.sel.ranges
      ...
      getTokenTypeAt: function (pos) {
        ...
        let before = 0; let after = (styles.length - 1) / 2; const ch = pos.ch
        let type
      ...
      getHelpers: function (pos, type) {
        ...
        const help = helpers[type]; const mode = this.getModeAt(pos)
      ...
      cursorCoords: function (start, mode) {
        let pos; const range = this.doc.sel.primary()
      ...
      addWidget: function (pos, node, scroll, vert, horiz) {
        ...
        let top = pos.bottom; let left = pos.left
      ...
      moveH: methodOp(function (dir, unit) {
        ...
        this.extendSelectionsBy(function (range) {
          if (this$1.display.shift || this$1.doc.extend || range.empty()) { return findPosH(this$1.doc, range.head, dir, unit, this$1.options.rtlMoveVisually) } else { return dir < 0 ? range.from() : range.to() }
        }, sel_move)
      }),

      deleteH: methodOp(function (dir, unit) {
        const sel = this.doc.sel; const doc = this.doc
      ...
      findPosV: function (from, amount, unit, goalColumn) {
        let dir = 1; let x = goalColumn
        if (amount < 0) { dir = -1; amount = -amount }
        let cur = clipPos(this.doc, from)
        for (let i = 0; i < amount; ++i) {
          const coords = cursorCoords(this, cur, 'div')
      ...
      moveV: methodOp(function (dir, unit) {
        ...
        const doc = this.doc; const goals = []
      ...
      findWordAt: function (pos) {
        const doc = this.doc; const line = getLine(doc, pos.line).text
        let start = pos.ch; let end = pos.ch
        if (line) {
      ...
      setSize: methodOp(function (width, height) {
        ...
        this.doc.iter(lineNo, this.display.viewTo, function (line) {
          if (line.widgets) {
            for (let i = 0; i < line.widgets.length; i++) { if (line.widgets[i].noHScroll) { regLineChange(this$1, lineNo, 'widget'); break } }
          }
          ++lineNo
        })
        this.curOp.forceUpdate = true
  ...
  function findPosH (doc, pos, dir, unit, visually) {
    ...
    const lineDir = visually && doc.direction == 'rtl' ? -dir : dir
    ...
    function moveOnce (boundToLine) {
      let next
      if (unit == 'codepoint') {
        const ch = lineObj.text.charCodeAt(pos.ch + (dir > 0 ? 0 : -1))
        if (isNaN(ch)) {
          next = null
        } else {
          const astral = dir > 0 ? ch >= 0xD800 && ch < 0xDC00 : ch >= 0xDC00 && ch < 0xDFFF
    ...
    if (unit == 'char' || unit == 'codepoint') {
      ...
      let sawType = null; const group = unit == 'group'
  ...
  function findPosV (cm, pos, dir, unit) {
    const doc = cm.doc; const x = pos.left; let y
    if (unit == 'page') {
  ...
  ContentEditableInput.prototype.init = function (display) {
    ...
    const input = this; const cm = input.cm
    ...
    function onCopyCut (e) {
      ...
      const kludge = hiddenTextarea(); const te = kludge.firstChild
      cm.display.lineSpace.insertBefore(kludge, cm.display.lineSpace.firstChild)
      te.value = lastCopied.text.join('\n')
      const hadFocus = activeElt()
      selectInput(te)
      setTimeout(function () {
  ...
  ContentEditableInput.prototype.showPrimarySelection = function () {
    const sel = this.getSelection(); const cm = this.cm; const prim = cm.doc.sel.primary()
    const from = prim.from(); const to = prim.to()
    ...
    const old = sel.rangeCount && sel.getRangeAt(0); let rng
  ...
  ContentEditableInput.prototype.pollSelection = function () {
    ...
    const sel = this.getSelection(); const cm = this.cm
  ...
  ContentEditableInput.prototype.pollContent = function () {
    ...
    const cm = this.cm; const display = cm.display; const sel = cm.doc.sel.primary()
    let from = sel.from(); let to = sel.to()
    ...
    let cutFront = 0; let cutEnd = 0
    const newTop = newText[0]; const oldTop = oldText[0]; const maxCutFront = Math.min(newTop.length, oldTop.length)
    while (cutFront < maxCutFront && newTop.charCodeAt(cutFront) == oldTop.charCodeAt(cutFront)) { ++cutFront }
    const newBot = lst(newText); const oldBot = lst(oldText)
  ...
  function posToDOM (cm, pos) {
    ...
    const order = getOrder(line, cm.doc.direction); let side = 'left'
    if (order) {
  ...
  function domTextBetween (cm, from, to, fromLine, toLine) {
    let text = ''; let closing = false; const lineSep = cm.doc.lineSeparator(); let extraLinebreak = false
    ...
    function walk (node) {
      if (node.nodeType == 1) {
        ...
        const markerID = node.getAttribute('cm-marker'); let range
        if (markerID) {
  ...
  function locateNodeInLineView (lineView, node, offset) {
    const wrapper = lineView.text.firstChild; let bad = false
    ...
    let textNode = node.nodeType == 3 ? node : null; let topNode = node
    if (!textNode && node.childNodes.length == 1 && node.firstChild.nodeType == 3) {
      textNode = node.firstChild
      if (offset) { offset = textNode.nodeValue.length }
    }
    while (topNode.parentNode != wrapper) { topNode = topNode.parentNode }
    const measure = lineView.measure; const maps = measure.maps
  ...
  TextareaInput.prototype.init = function (display) {
    ...
    const input = this; const cm = this.cm
  ...
  TextareaInput.prototype.prepareSelection = function () {
    ...
    const cm = this.cm; const display = cm.display; const doc = cm.doc
    const result = prepareSelection(cm)

    // Move the hidden textarea near the cursor to prevent scrolling artifacts
    if (cm.options.moveInputWithCursor) {
      const headPos = cursorCoords(cm, doc.sel.primary().head, 'div')
      const wrapOff = display.wrapper.getBoundingClientRect(); const lineOff = display.lineDiv.getBoundingClientRect()
  ...
  TextareaInput.prototype.showSelection = function (drawn) {
    const cm = this.cm; const display = cm.display
  ...
  TextareaInput.prototype.fastPoll = function () {
    let missed = false; const input = this
  ...
  TextareaInput.prototype.poll = function () {
    ...
    const cm = this.cm; const input = this.textarea; let prevInput = this.prevInput
    ...
    let same = 0; const l = Math.min(prevInput.length, text.length)
  ...
  TextareaInput.prototype.onContextMenu = function (e) {
    const input = this; const cm = input.cm; const display = cm.display; const te = input.textarea
    if (input.contextMenuPending) { input.contextMenuPending() }
    const pos = posFromMouse(cm, e); const scrollPos = display.scroller.scrollTop
    ...
    const oldCSS = te.style.cssText; const oldWrapperCSS = input.wrapper.style.cssText
...
(function (mod) {
  ...
  CodeMirror.defineMode('markdown', function (cmCfg, modeCfg) {
    ...
    const setextHeaderRE = /^ {0,3}(?:\={1,}|-{2,})\s*$/
    const textRE = /^[^#!\[\]*_\\<>` "'(~:]+/
