class NewPostWizard {
    ...
    createStepIndicator() {
        return `
            <ul class="nav nav-pills mb-4 justify-content-center">
                <li class="nav-item">
                    <button class="nav-link ${this.currentStep === 1 ? 'active' : ''}" data-step="1">
                        Basic Info
                    </button>
                </li>
                <li class="nav-item">
                    <button class="nav-link ${this.currentStep === 2 ? 'active' : ''}" data-step="2">
                        Thumbnail & Author
                    </button>
                </li>
                <li class="nav-item">
                    <button class="nav-link ${this.currentStep === 3 ? 'active' : ''}" data-step="3">
                        Categories & Tags
                    </button>
                </li>
            </ul>`;
    ...
    createThumbnailStep() {
        return ` <div id="imageSelectorContainer"></div>
            <div class="step-content">
                <div class="mb-3">
                    ...
                    <div class="tags-container">
                        ...
                        <div class="existing-tags mt-2">
                            ${this.existingTags.map(tag => `
                                <button type="button" 
                                        class="btn btn-sm ${selectedTags.has(tag.name) ? 'btn-primary' : 'btn-outline-primary'} me-1 mb-1 toggle-tag"
                                        data-tag="${tag.name}">
                                    ${tag.name}
                                </button>
                            `).join('')}
                    ...
                    <div class="categories-container">
                        ...
                        <div class="existing-categories mt-2">
                            ${this.existingCategories.map(category => `
                                <button type="button" 
                                        class="btn btn-sm ${selectedCategories.has(category.name) ? 'btn-secondary' : 'btn-outline-secondary'} me-1 mb-1 toggle-category"
    ...
    attachEventListeners() {
        ...
        this.container.querySelector('.next-step')?.addEventListener('click', () => this.nextStep());
        this.container.querySelector('.prev-step')?.addEventListener('click', () => this.previousStep());

        this.container.querySelector('.submit-post')?.addEventListener('click', () => this.submit());
        ...
        this.container.querySelectorAll('.remove-category').forEach(removeIcon => {
            removeIcon.addEventListener('click', (e) => {
                const category = e.target.getAttribute('data-category');
